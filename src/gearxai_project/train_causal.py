...
def _causal_relevance_loss(
    ...
    fraction_mode = str(train_cfg.get("causal_fraction_mode", "sample")).lower()
    if fraction_mode == "sample":
        active_fractions = [float(fractions[np.random.randint(0, len(fractions))])]
    elif fraction_mode == "mean":
        active_fractions = fractions
    else:
        raise ValueError(f"Unsupported causal_fraction_mode: {fraction_mode!r}")
    ...
    delete_objective = str(train_cfg.get("causal_delete_objective", "margin")).lower()
    insert_objective = str(train_cfg.get("causal_insert_objective", "margin")).lower()

    target_mode = str(train_cfg.get("causal_target_mode", "label")).lower()
    if target_mode == "predicted":
        # Match official faithfulness evaluation: preserve/expose the model's
        # originally predicted class rather than the ground-truth class.
        label_index = logits.detach().argmax(dim=1, keepdim=True)
    elif target_mode == "label":
        label_index = y.unsqueeze(1)
    else:
        raise ValueError(f"Unsupported causal_target_mode: {target_mode!r}")
    ...
    for fraction in active_fractions:
        ...
        if delete_objective == "margin":
            # Deleted confidence should be lower than full confidence by at least margin.
            delete_losses.append(F.relu(deleted_conf - full_conf + delete_margin).mean())
        elif delete_objective == "auc":
            # Official deletion AUC is the average deleted confidence along the curve.
            delete_losses.append(deleted_conf.mean())
        else:
            raise ValueError(f"Unsupported causal_delete_objective: {delete_objective!r}")

        if insert_objective == "margin":
            # Inserted confidence should stay close to full confidence.
            insert_losses.append(F.relu(full_conf - inserted_conf - insert_tolerance).mean())
        elif insert_objective == "auc":
            # Maximizing insertion AUC is equivalent to minimizing negative confidence.
            insert_losses.append(-inserted_conf.mean())
        else:
            raise ValueError(f"Unsupported causal_insert_objective: {insert_objective!r}")

    delete_loss = torch.stack(delete_losses).mean() if delete_losses else logits.new_tensor(0.0)
    insert_loss = torch.stack(insert_losses).mean() if insert_losses else logits.new_tensor(0.0)

    total = delete_weight * delete_loss + insert_weight * insert_loss
    mean_fraction = float(np.mean(active_fractions)) if active_fractions else 0.0
    return total, delete_loss, insert_loss, mean_fraction


def _freeze_batchnorm_stats(module: nn.Module) -> None:
    """Keep pretrained BatchNorm running statistics fixed during causal fine-tuning."""
...
def train_one_epoch(
    ...
    model.train()
    if bool(train_cfg.get("freeze_batchnorm_stats", False)):
        model.apply(_freeze_batchnorm_stats)
